...
This plot shows that an mtry value of 4 results in the highest accuracy
...
gyroscopic information from the dumbell, arm and forearm in mainly the
...
component that is dominanted by information from the arm/foremarm/dumbbell
...
~98%. Thus the model has an out of sample error of 2 percent. Thus, with
the test data set the model should make 0 or maybe 1 error given there are
20 samples in the test set. This model seems to do an excellent job of
classifying the quality of an exercise based on accelerometer data.
